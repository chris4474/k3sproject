...
sudo parted -s $mydev mkpart primary fat32 0% 512MB # partition de boot 512MB
sudo parted -s $mydev set  1 boot on # rendre la première partition bootable
sudo parted -s $mydev mkpart primary ext4 512MB 55% # 
...
# can be verified with vcgencmd get_mem gpu
